...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        sample_mean = np.mean(x, axis=0, keepdims=True)  # mean of x_i
        sample_var = np.var(x, axis=0, keepdims=True)  # var of x_i
        x_normalized = (x-sample_mean)/np.sqrt(sample_var + eps)  # normalize
        out = gamma * x_normalized + beta  # out

        cache = (x_normalized, gamma, beta, sample_mean, sample_var, x, eps)  # store the cache
